...
public class DateUtil {
  ...
  public static final FastDateFormat YEARMONTH_FORMAT = FastDateFormat
      .getInstance("yyyyMM");
  ...
  public static final boolean isLimitDate(String startDate, String endDate,
    ...
    return startTrainDate.compareTo(startDate) >= 0 && startTrainDate.compareTo(endDate) <= 0;
  }
   ...
   * @param startTrainDate 指定日期
   */
  public static final boolean isLimitDate(String startYear,
      String startMonth, String endYear, String endMonth, String startTrainDate) {
    StringBuilder startDate = new StringBuilder(startYear);
    startDate.append(startMonth);
    StringBuilder endDate = new StringBuilder(endYear);
    endDate.append(endMonth);
    return isLimitDate(startDate.toString(), endDate.toString(), startTrainDate);
  }

  /**
   * 验证开始时间是否晚于结束时间
   * 开始时间与结束时间只填写一个时,默认返回true
   *
   * @param startDate 开始时间
   * @param endDate 结束时间
   * @return boolean
   */
  public static final boolean compareToDate(final Date startDate, final Date endDate) {
